...
FileProcessor.prototype.replaceWithRevved = function replaceWithRevved(lines, assetSearchPath) {
  ...
  regexps.forEach(function (rxl) {
    ...
    content = content.replace(rxl[0], function (match, src) {
      ...
      // srcFileObject has the struct {srcFile:'',prefix:''} which saves the prefix of srcFile used to prepend in filterOut function
      var srcFile;
      var srcFileObject = filterIn(src);
      if(srcFileObject.srcFile){
          srcFile = srcFileObject.srcFile;
      }else{
        srcFile = srcFileObject;
      }
      ...
      var res = match.replace(src, filterOut(file,self.file,match,src,srcFileObject));
      if (srcFile !== file) {
