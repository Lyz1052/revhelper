      ...
      filePair.src.forEach(function(src) {
        ...
        if (grunt.file.isDir(src)) {
          ...
          //handle duplicate files
            var ext = path.extname(src).replace(/\./g,'');
            if(filePair.orig.deletesame&&filePair.orig.deleteskip){
                var skips = filePair.orig.deleteskip.split(",");
                if(skips.indexOf(ext)==-1 && path.basename(dest) === path.basename(src)){
                   //delete dest file and mapping if exists
                        ...
                        if(grunt.filerev&&grunt.filerev.summary){
                          ...
                          var removeProps = [];
                            for(var prop in summary){
                              if(summary[prop].replace(/\\\\/g,"/").replace(/\\/g,"/")==dest){
